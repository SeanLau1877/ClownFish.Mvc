...
namespace ClownFish.Mvc.Proxy
{
	...
	public class ProxyTransferHandler : HttpTaskAsyncHandler
	{
		...
		/// </summary>
		internal static readonly string TargetUrlKeyName = "target-url";
		...
		internal static readonly string ProxyFlagHeader = "x-ClownFish.Mvc-Proxy";
		...
		protected virtual void CopyResponseHeaders(HttpContext context, HttpWebResponse webResponse)
		{
			...
			if( context.Request.Browser.Browser == "Safari" ) {	
				...
				string filename = headers["Content-Disposition-proxy"];
				if( string.IsNullOrEmpty(filename) == false ) {
					filename = HttpUtility.UrlDecode(filename);						// 从备用的响应头拿到正确的文件名

					context.Response.Headers.Remove("Content-Disposition-proxy");	// 删除这个头，因为对用户浏览器没有用。
